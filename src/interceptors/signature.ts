...
function hmac(key: any, string: string, encoding: any) {
...
function encodeRfc3986Full(str: any): string {
	return str;
...
class RequestSigner {
	...
	canonicalString() {
		...
		if (query) {
			let reducedQuery: Query = Object.keys(query).reduce(function (
				obj: Query,
				key
			) {
				if (!key) {
		...
		if (pathStr !== "/") {
			...
			pathStr = pathStr
				.split("/")
				.reduce(function (path: string[], piece: string) {
	...
	canonicalHeaders() {
		...
		return Object.keys(headers)
			.filter(function (key) {
				...
				if (keyLowerCase in HEADERS_TO_IGNORE) {
					return false;
				} else {
...
export function sign(request: any) {
